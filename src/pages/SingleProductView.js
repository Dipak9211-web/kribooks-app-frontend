...
function SingleProductView() {
  ...
  return (
    ...
    <Badge.Ribbon text={`${product?.sold} sold` } color="red">
       <Badge.Ribbon placement="start" color="green" text={`${product?.quantity >= 1? `${product?.quantity - product?.sold} in stock`:"Out of stock"} `}>
        {product._id &&  <img className="card-img-top" src={`${process.env.REACT_APP_API}/product/photo/${product._id}`} alt={product.name}  
       style={{height:'500px', width:"100%", objectFit:'cover'}} 
   /> }
       </Badge.Ribbon>
    </Badge.Ribbon>
   <div className="card-body">
   <h1 className='fw-bold'>{product?.name} sold</h1>
   <p className="card-text lead">{product?.description}</p>
   </div>
         <div className="d-flex justify-content-between lead p-5 fw-bold bg-light">
            <div>
              ...
              <p><FaProjectDiagram/> Categories: {product?.category?.name} </p>
               <p>
